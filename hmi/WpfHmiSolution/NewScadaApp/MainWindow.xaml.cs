...
namespace NewScadaApp
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private void Client_MqttMsgPublishReceived(object sender, MqttMsgPublishEventArgs e)
        {
            try
            {
                // 대리자에게 대신 UI스레드에 속한 컨트롤에 일처리 호출
                Dispatcher.Invoke(DispatcherPriority.Normal, new Action(delegate
                {
                    ...
                    if (currData["dev_addr"] == "4001")  // Tank로부터 데이터 수신
                    {
                        if (currData["code"] == "red" && currData["value"] == "1") // 경고등 온
                        {
                            LedState.CurrState = Color.FromRgb(255, 0, 0);
                            MessageBox.Show("비상상황! FuelTank 모터를 구동하십시오!");
                        }
                        else if (currData["code"] == "green" && currData["value"] == "1") // 정상동작등 온
                        {
                            LedState.CurrState = Color.FromRgb(0, 255, 0);
                        }
                        else if (currData["value"] == "0") // LED 오프
                        {
        ...
        private void BtnMotor_CustomClick(object sender, RoutedEventArgs e)
        {
            if (client.IsConnected)
            {
                ...
                string pubData = "{\n " +
                                 ...
                                 "\t   \"value\" : \"1\" \n" +
                                 "} ";
                client.Publish("Kasan01/4002/", Encoding.UTF8.GetBytes(pubData), MqttMsgBase.QOS_LEVEL_EXACTLY_ONCE, false);
