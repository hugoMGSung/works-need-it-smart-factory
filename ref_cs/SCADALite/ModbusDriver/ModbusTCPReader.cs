...
namespace ModbusDriver
{
    ...
    public sealed class ModbusTCPReader : IPLCDriver, IMultiReadWrite
    {
        ...
        public int PDU
        {
            ...
            /* 수정인：yjz
              수정일자：20171125
              업데이트 이유: modbus-TCP에 규정된 프로토콜.
                        ADU=MBAP+기능 코드+데이터 여기서 ADU는 256바이트, MBAP는 7바이트, 기능 코드는 1바이트, 데이터는 248바이트.
                        PDU=기능 코드+데이터
                        따라서 PDU는 249바이트
             */
            get { return 249; } // 0xF9 = 249
        }
        ...
        private byte[] WriteSyncData(byte[] write_data)
        {
            ...
            else
            {
                lock (_async)
                {
                    try
                    {
                        tcpSynCl.Send(write_data, 0, write_data.Length, SocketFlags.None);// LOCK problem??
