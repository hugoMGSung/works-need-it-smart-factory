...
namespace FileDriver
{
    public class FileDeviceGroup : IGroup
    {
        ...
        {
            int index = data.IndexOf('#'); // !!!체제：GroupID#TagID:Value|TagID:Value|TagID:Value
                    ...
                    if (ind > 0)
                    {
                        ...
                        if (short.TryParse(data.Substring(0, ind), out tid))
                        {
                            ...
                            if (tag != null)
                            {
                                ...
                                tag.Update(value, date, QUALITIES.QUALITY_GOOD);// 값 전달불가 Tag 자체 refresh
                                hdata[i].ID = tag.ID;
                                hdata[i].Value = value;
