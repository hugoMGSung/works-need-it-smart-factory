...
namespace DataService
{
    public static class ExtMethods
    {
                ...
                if (count > 1)
                {
                    ...
                    for (int j = 1, i = 1; i < count; i++, j++)
                    {
                        ...
                        if (offset1 > (PDU / cacheReader.ByteCount))
                        {
                            ...
                            startIndex += j; j = 0;
                            cacheLength += len; // 캐시 길이 업데이트
                            cacheIndexStart = cacheLength;
                            segmentStart = tagAddress; // 데이터 세그먼트의 시작 주소 업데이트
                        }
                        else
                        {
                    ...
                    cacheReader.Size = start.DataSize <= bitCount ? 1 : start.DataSize / bitCount; // Cache의 Size 속성 값을 변경하면 Cache의 메모리 영역이 생성
            }
            return rangeList;
        }
        ...
        public static int PLCWriteMultiple(this IPLCDriver plc, ICache cache, DeviceAddress[] addrArr, object[] buffer, int limit)
        {
            ...
            lock (plc) // 잠금을 해제하면 동시성 충돌이 발생하고 잠금도 절대적인 보안을 보장할 수 없습니다.누군가 현장 작업을 수행하면 데이터가 갱신
            {
    ...
    public static class Utility
    {
        ...
        /// <summary>
        /// 데이터 문자열을 취득을 위해 헤더에서 엔드까지 가져오기 
        /// </summary>
        /// <param name="allStr">전체문자열</param>
        /// <param name="startStr">시작문자열</param>
        /// <param name="endStr">완료문자열</param>
        /// <returns></returns>
        ...
        /// XOR 검사
        /// </summary>
        /// <param name="xorStr">확인문자열</param>
        /// <returns>코드확인</returns>
        public static string XorCheck(string xorStr)
        {

            try
            {
                // XOR 검사는 대소문자를 구분하므로 소문자는 대문자로 변환
                /*****************************************************
                * XOR 체크
                * VerifyByte는 획득한 인증 코드
                ***************************************************/
