    ...
    public class DAService : IDataServer
    {
        ...
        private void timer1_Elapsed(object sender, ElapsedEventArgs e)
        {
            foreach (IDriver d in Drivers)
            {
                if (d.IsClosed)
                {
                    d.Connect(); // t.IsAlive 판단에 추가가능 스레드 비정상이면 재시작
                }
            }
        }
        ...
        void InitConnection()
        {
            foreach (IDriver reader in _drivers.Values)
            {
                ...
                foreach (IGroup grp in reader.Groups)
                {
                    ...
                    // 판단 추가가능
                    // ClientDriver에서 전송하면 변경된 데이터 브로드캐스트할 필요없고 보관
                    grp.IsActive = grp.IsActive;
                }
            }
            // 여기서 개선 필요, Condition과 동일한 처리 방식, 구성 가능
        }
        ...
        public IDriver AddDriver1(short id, string name, string assembly, string className, JObject target)
        {
            ...
            switch (className)
            {
                case "FileDriver.TagDriver":
                    ...
                    _drivers.Add(id, dv1);
                    return dv1;
                case "ModbusDriver.ModbusTCPReader": // todo: 나중에 개선 필요
                    ModbusTCPReader dv2 = new ModbusTCPReader(this, id, name)
                    {
                        ServerName = target["ServerName"].ToString(),
                        Port = int.Parse(target["Port"].ToString()),
                        TimeOut = int.Parse(target["TimeOut"].ToString()),
                        SlaveID = short.Parse(target["SlaveID"].ToString())
                    };
                    _drivers.Add(id, dv2);
                    return dv2;
                case "ModbusDriver.ModbusRTUReader":
                    ...
                    {
                        PortName = target["PortName"].ToString(),
                        TimeOut = int.Parse(target["TimeOut"].ToString()),
                        SlaveID = short.Parse(target["SlaveID"].ToString()),
                        BaudRate = int.Parse(target["BaudRate"].ToString()),
                        DataBits = int.Parse(target["DataBits"].ToString()),
                        StopBits = (StopBits)int.Parse(target["StopBits"].ToString()),
                        Parity = (Parity)int.Parse(target["Parity"].ToString())
                    };
                    _drivers.Add(id, dv3);
                    return dv3;
                default:
